...
mutable struct Client{T1<:Int64, T2<:Float64, T3<:Vector{T1}, T4<:Matrix{T2}, T5<:SparseMatrixCSC{T2, T1}}
    ...
    function Client(id::Int64, Xtrain::SparseMatrixCSC{Float64, Int64}, Xtest::SparseMatrixCSC{Float64, Int64}, config::Dict{String, Union{Int64, Float64}})
        ...
        new{Int64, Float64, Vector{Int64}, Matrix{Float64}, SparseMatrixCSC{Float64, Int64}}(id, Xtrain, Xtest, num_classes, num_clients, num_epoches, batch_size, learning_rate, W, batch, grads)
    end
end
...
mutable struct AsynClient{T1<:Int64, T2<:Float64, T3<:Matrix{T2}, T4<:SparseMatrixCSC{T2, T1}}
    ...
    function AsynClient(id::Int64, Xtrain::SparseMatrixCSC{Float64, Int64}, Xtest::SparseMatrixCSC{Float64, Int64}, ts::Float64, config::Dict{String, Union{Int64, Float64}})
        num_classes = config["num_classes"]
        num_clients = config["num_clients"]
        batch_size = config["batch_size"]
        learning_rate = config["learning_rate"]
        ...
        grads = zeros(Float64, num_classes, batch_size )
        num_commu = 0
        new{Int64, Float64, Matrix{Float64}, SparseMatrixCSC{Float64, Int64}}(id, Xtrain, Xtest, num_classes, num_clients, batch_size, learning_rate, W, grads, ts, num_commu)
    end
end
